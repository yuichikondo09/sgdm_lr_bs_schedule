...
|`nshb`| `boolean` (e.g., `true`) | Selects the momentum update rule. <br> `false` to use SHB: $m_t = \beta m_{t-1} + \nabla f(\theta_{t})$ <br> `true` to use NSHB: $m_t = \beta m_{t-1} + (1-\beta) \nabla f(\theta_{t})$|
| `use_wandb` | `boolean` (e.g., `true`) | Enables logging to Weights & Biases (wandb). |
